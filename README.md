...
### v1.2 (Latest)
* **실시간 수집 중단 제어**: 수집 도중 언제든지 **'중단' 버튼**을 눌러 작업을 안전하게 멈출 수 있는 제어 기능을 추가했습니다.
* **대량 수집 지원**: 최대 다운로드 가능 수량을 **1,000개**로 상향하여 대규모 논문 수집이 가능합니다.
* **수집 카테고리 대폭 확장**: 기존의 학술/학위 논문 외에도 **학술지, 단행본, 연구보고서** 등 다양한 유형의 자료를 선택하여 수집할 수 있도록 지원 범위를 넓혔습니다.
* **엄격한 PDF 감지**: 중간 단계의 HTML 페이지(`.do` 등)를 걸러내고, 실제 `.pdf` 파일 주소가 확인될 때만 다운로드를 실행하여 수집 정확도를 높였습니다.
* **지능형 자동 탭 관리**: 다운로드 시작 5초 후 뷰어 팝업창을 자동으로 닫아 브라우저 정돈 및 메모리 효율을 개선했습니다.
* **ezPDF 뷰어 완벽 대응**: `?file=` 파라미터 내부에 PDF 주소가 숨겨진 특수 뷰어 환경에서도 원본 파일을 정상적으로 추출합니다.
...
6. **수집 중단**: 과정 중에 중단하고 싶다면 언제든지 **'중단'** 버튼을 클릭하여 작업을 안전하게 멈출 수 있습니다.
